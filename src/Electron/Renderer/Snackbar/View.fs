...
module View =
    ...
    let private view' (classes: IClasses) model dispatch =
        snackbar
            [ yield AnchorOrigin
                        ({ vertical = SnackbarVerticalOrigin.Bottom
                           horizontal = SnackbarHorizontalOrigin.Left })
              yield Open
              ...
              match model.State with
              | Active x
              | Waiting x -> yield Message(str x.Message)
              | _ -> ()
              match model.State with
                  ...
                  yield SnackbarProp.Action
                            (button
                                [ Key action
                                  ...
                                  ButtonProp.Variant ButtonVariant.Contained ] [ str action ])
              | _ -> () ] []
