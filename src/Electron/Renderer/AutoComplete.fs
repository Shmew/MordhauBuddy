...
module AutoComplete =
    ...
    let private styles (theme : ITheme) : IStyles list =
        [ Styles.Custom("formControl", [ MarginBottom(theme.spacing.unit * 2) ])
          Styles.Custom("menu",
                        [ MaxHeight "500px"
                          Overflow "auto" ])
          Styles.Custom("noResults", [ CSSProp.Padding(theme.spacing.unit * 2) ])
          Styles.Custom("selectedItem", [ CSSProp.FontWeight "bold" ])
          Styles.Custom("highlight", [ CSSProp.FontWeight "bold" ]) ]
