    ...
    let private profiles (classes: IClasses) model dispatch =
        let createCard (direction: ToggleDirection) =
            let checkNum, profiles, headerTitle =
                ...
                | Left -> model.TransferList.LeftChecked, model.TransferList.LeftProfiles, "Profile List"
                | Right -> model.TransferList.RightChecked, model.TransferList.RightProfiles, "Modification List"
                      ...
                      [ cardHeader
                          [ checkbox
                              [ DOMAttr.OnClick <| fun _ ->
                                  (direction, checkNum = 0 && checkNum <> pLen)
                                  |> ToggleAll
                                  |> dispatch
              ...
              grid
                  ...
                  [ button
                      [ ButtonProp.Variant ButtonVariant.Contained
                        ...
                        HTMLAttr.Disabled(model.TransferList.LeftChecked = 0) ] [ str ">" ]
                    button
                        [ ButtonProp.Variant ButtonVariant.Contained
                          MaterialProp.Color ComponentColor.Secondary
                          ButtonProp.Size ButtonSize.Small
                          ...
                          Style [ CSSProp.MarginTop "1em" ] ] [ str "<" ] ]
              createCard Right ]

    let private actionTabs (classes: IClasses) model dispatch =
        let tabDescription =
            match model.TabSelected with
            | Tab.Frankenstein -> str "Frankenstein mode is done automatically \
                    and will select at random the maximum or \
                    minimum allowed values for each aspect of \
                    the character's face."
            | Tab.Random -> str "Random mode is done automatically and \
                    will select at random all aspects of the \
                    character's face."
            | Tab.Import -> str "Import mode enables you to set faces from a face value string."
            | Tab.Export -> str "Export lets you generate string values of the profiles you've selected."
                                ...
                                [ HTMLAttr.Disabled model.Import.State.IsImportSuccess
                                  ...
                                  Style
                                      [ CSSProp.MarginLeft "1em"
                                        CSSProp.MaxHeight "4em" ] ] [ str "Validate" ] ]
                      div [ Style [ CSSProp.Padding "0em 5em" ] ]
                          [ expansionPanel [ MaterialProp.Elevation 2 ]
                                [ expansionPanelSummary [ ExpansionPanelSummaryProp.ExpandIcon <| expandMoreIcon [] ]
                                      [ str "An example of an import string" ]
                                  divider []
                                  expansionPanelDetails [ Style [ CSSProp.Padding "1em" ] ]
                                      [ code [ Style [ CSSProp.WordBreak "break-all" ] ] [ str Samples.faceImport ]
                                        button
                                            [ DOMAttr.OnClick <| fun _ ->
                                ...
                                [ for (index, profile) in profileInd do
                                    yield div []
                                              [ listItem []
                                                ...
                                                divider
                                                    [ HTMLAttr.Hidden
                                                        (index + 1 = model.TransferList.RightProfiles.Length) ] ] ] ] ]
