                      ...
                      yield menu
                                ...
                                [ clickAwayListener
                                    [ ClickAwayListenerProp.OnClickAway
                                      <| fun _ -> dispatch (ToggleMenu(model.TabSelected, map.Map.Folder)) ]
                                      [ div [] <| match model.TabSelected with
                                                  | Installed ->
                                                      [ menuItem
                                                          [ HTMLAttr.Disabled
                                                              (model.Available
                                                               |> List.exists (fun m -> m.Map.Folder = map.Map.Folder)
                                                               |> not)
                                                            DOMAttr.OnClick
                                                            <| fun _ -> dispatch (Install(map.Map.Folder)) ]
                                                            [ str "Update" ]
                                                        menuItem
                                                            [ DOMAttr.OnClick
                                                              <| fun _ -> dispatch (Uninstall(map.Map.Folder)) ]
                                                            [ str "Uninstall" ] ]
                                                  | Installing ->
                                                      [ menuItem
                                                          [ DOMAttr.OnClick
                                                            <| fun _ -> dispatch (CancelInstall(map.Map.Folder)) ]
                                                            [ str "Cancel" ] ]
                                                  | _ -> [] ] ]
                  | _ -> () ]
                          ...
                          [ CardHeaderProp.Title <| str (map.Map |> getName)
                            ...
                            <| fab
                                [ MaterialProp.Color ComponentColor.Secondary
                                  ...
                                  DOMAttr.OnClick <| fun _ -> dispatch (Install(map.Map.Folder)) ] [ addIcon [] ] ] []
                        cardMedia
                            [ match map.Map.Image with
                              | Some(mapImage) ->
                                  yield CardMediaProp.Image mapImage
                                  yield HTMLAttr.Title <| getName map.Map
                              | _ -> yield HTMLAttr.Title "No picture provided"
                              yield Style
                                        [ CSSProp.Width "50em"
                                          CSSProp.Height "20em" ] ]
                        cardContent []
        ...
        |> List.map (fun map ->
            tableRow []
                [ tableCell [ TableCellProp.Align TableCellAlign.Left ]
                      [ typography
                          [ if map.State.IsStateError then yield TypographyProp.Color TypographyColor.Error
                            yield TypographyProp.Variant TypographyVariant.Body2 ] [ str (map.Map |> getName) ] ]
                  tableCell
                  ...
                  tableCell [ TableCellProp.Align TableCellAlign.Right ]
                      [ typography
                          [ if map.State.IsStateError then yield TypographyProp.Color TypographyColor.Error
                            yield TypographyProp.Variant TypographyVariant.Body2 ] [ map.Map.Author |> defStr ] ]
                  tableCell
                      [ TableCellProp.Align TableCellAlign.Right
                        Style [ CSSProp.MinWidth "8em" ] ]
                      [ typography
                          [ if map.State.IsStateError then yield TypographyProp.Color TypographyColor.Error
                            yield TypographyProp.Variant TypographyVariant.Body2 ] [ map.Map.GetDate() |> defStr ] ]
                  tableCell [ TableCellProp.Align TableCellAlign.Right ]
                      [ typography
                          [ if map.State.IsStateError then yield TypographyProp.Color TypographyColor.Error
                  ...
                  tableCell
                      ...
                      [ typography
                          [ if map.State.IsStateError then yield TypographyProp.Color TypographyColor.Error
                            yield TypographyProp.Variant TypographyVariant.Body2 ]
                            [ map.Map.FileSize
                              |> Option.map (fun v -> sprintf "%s %s" (v |> string) "MB")
                              |> defStr ] ]
                  tableCell
                      [ TableCellProp.Align TableCellAlign.Right
                        Style [ CSSProp.MinWidth "6em" ] ]
                      [ typography
                          [ if map.State.IsStateError then yield TypographyProp.Color TypographyColor.Error
                            yield TypographyProp.Variant TypographyVariant.Body2 ] [ map.Map.GetPlayers() |> defStr ] ]
                  tableCell
                      [ TableCellProp.Align TableCellAlign.Right
                        Style [ CSSProp.Width "1em" ] ] [ tableMenu classes model dispatch map ] ]
            |> fun tRow ->
                match map.State with
                | ComMapState.Error(errMsg) -> addMapErrorTooltip tRow errMsg
                | _ -> tRow)
                                 ...
                                 [ tableHead []
                                       [ tableRow []
                                             [ tableCell [ TableCellProp.Align TableCellAlign.Left ] [ str "Name" ]
                                               ...
                                               tableCell [ TableCellProp.Align TableCellAlign.Right ]
                                                   [ str "File Size" ]
                                               tableCell [ TableCellProp.Align TableCellAlign.Right ] [ str "Players" ]
                                               tableCell [ TableCellProp.Align TableCellAlign.Right ] [] ] ]
                          ...
                          div
                              [ Style
                                  [ CSSProp.Padding "2em"
                                    ...
                                    CSSProp.OverflowY "Scroll" ] ] [ tabContent classes model dispatch ] ]
              yield div
                        [ Style
