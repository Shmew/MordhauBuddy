...
module FSharpLinter =
    ...
    let lintFiles (file: string) fileList =
        let lintFile (file: string) =
            let sw = Diagnostics.Stopwatch.StartNew()
            try
                Lint.lintFile parseInfo file |> handleLintResult
                sw.Stop()
                { Path = file
        ...
        |> List.map lintFile
        |> buildLintResults
        |> LintResults.writeNUnitSummary file
