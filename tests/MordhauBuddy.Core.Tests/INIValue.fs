...
module INIValue =
    ...
    module Utils =
        ...
        let kvPair s1 s2 =
            INIValue.KeyValue(s1,iString s2)
        let kvPairNestedOne s nest =
            INIValue.KeyValue(s,iTupleOne nest)
        let kvPairNested s nest =
            INIValue.KeyValue(s,nest)
        let section s kvList =
            INIValue.Section(s,kvList)
        let sectionOne s iVal =
            INIValue.Section(s,[iVal])
        let fieldText s1 s2 =
            INIValue.FieldText(s1,iString ("\"" + s2 + "\"") |> iTupleOne)
        let serialI iVal =
            iVal.ToString()
    ...
    module OptionTests =
        ...
        let map (oVal: INIValue) (selectors: string list) (nVal: INIValue) =
            oVal.Map(selectors,nVal)
