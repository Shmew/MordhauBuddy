...
module GeneratorsCode =
    ...
    let miscWhitespace = [ '\u0009'; '\u000A'; '\u000B'; '\u000C'; '\u000D'; '\u0085'; '\u00A0' ]
    let whiteSpace = List.concat [ spaceSeparator; lineSeparator; paragraphSeparator; miscWhitespace ]

    let nonDigitalString() =
        gen { let! a = Arb.generate<NonEmptyString>
    ...
    let whitespaceString() =
        let length =
            ...
            |> List.head
            |> int
        Gen.arrayOfLength length <| Gen.elements whiteSpace |> Gen.map (fun x -> string (x))

    let nonEmptyNonAllWhitespaceString() =
        gen { return! Arb.generate<NonEmptyString> }
        |> Gen.filter (fun x ->
    ...
    let genDigitsInWhiteSpace() =
        gen { let! frontWhitespace = whitespaceString()
    ...
    let validDigits digits length =
        if digits.ToString().Length = length then digits.ToString()
        elif digits.ToString().Length < length then digits.ToString().PadLeft(length, '0')
        else digits.ToString().Substring(0, length)

    let invalidDigits digits length =
        if digits.ToString().Length = length then sprintf "0%s" <| digits.ToString()
    ...
    let inputZip5Plus4() =
        gen { let! digits = Arb.generate<NonNegativeInt>
    ...
    let genCallingCode() =
        let callingCode digits length =
            ...
            if x.StartsWith("0") then x.Replace("0", "1")
            else x
            |> UInt16.Parse
        gen {
            let! digits = Arb.generate<NonNegativeInt>
            let! callingCode = Gen.elements [ callingCode digits 4 |> Some
                                              callingCode digits 3 |> Some
                                              callingCode digits 2 |> Some
                                              callingCode digits 1 |> Some
                                              None ]
            return callingCode
        }
    ...
    let genPhoneNumber() =
        gen {
            ...
            let! extension = Gen.elements [ extensionRaw.ToString() |> Some
                                            None ]
            ...
            let! whiteSpace1 = Gen.elements [ whiteSpace1Raw |> Some
                                              None ]
            let! whiteSpace2 = Gen.elements [ whiteSpace2Raw |> Some
                                              None ]
            let! whiteSpace3 = Gen.elements [ whiteSpace3Raw |> Some
                                              None ]
            let! whiteSpace4 = Gen.elements [ whiteSpace4Raw |> Some
                                              None ]
            let! whiteSpace5 = Gen.elements [ whiteSpace5Raw |> Some
                                              None ]
            let symbolValueWhitespace symbol value whiteSpace =
                sprintf "%s%s%s%s" symbol (defaultArg whiteSpace String.Empty) value
                    (defaultArg whiteSpace String.Empty)
            let phoneNumber =
                sprintf "%s%s%s%s%s%s" (defaultArg whiteSpace1 String.Empty) (match callingCode with
                                                                              | Some x ->
                                                                                  symbolValueWhitespace "+" x
                                                                                      whiteSpace2
                                                                              | None -> String.Empty) phone
                    (defaultArg whiteSpace3 String.Empty) (match extension with
                                                           | Some x -> symbolValueWhitespace "X" x whiteSpace4
                                                           | None -> String.Empty) (defaultArg whiteSpace5 String.Empty)
    ...
    let genUri() =
        gen {
            ...
            let! uri = Gen.elements [ " http://" + uriRaw
                                      " https://" + uriRaw
                                      " ftp://" + uriRaw
                                      " ftps://" + uriRaw ]
            return uri
...
type Generators =
    static member NonEmptyStringList() =
        { new Arbitrary<string list>() with
              override __.Generator = GeneratorsCode.genNonEmptyNonAllWhitespaceStringList() }
